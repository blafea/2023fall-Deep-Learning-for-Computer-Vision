...
class QueryVideoDataset(Dataset):
    def __init__(self,
                 ...
                 clip_dir='student_data/clips',
                 meta_dir='student_data',
                 split='train',
    ...
    def _load_metadata(self):
        anno_processed_path = os.path.join(
            './data', '{}_anno_new.json'.format(self.split))
        if os.path.isfile(anno_processed_path):
            ...
            for clip_uid, clip_data in anno_json.items():
                for clip_anno in clip_data['annotations']:
                    for qset_id, qset in clip_anno['query_sets'].items():
                        ...
                        for frame_it in qset['response_track']:
                            response_track_frame_ids.append(
                                int(frame_it['frame_number']))
                        frame_id_min, frame_id_max = min(
                            response_track_frame_ids), max(response_track_frame_ids)
                        curr_anno = {
                            "clip_uid": clip_uid,
                            "query_set": qset_id,
                        ...
                        n_samples += 1
            print('Find {} data samples, {} valid (query path exist)'.format(
                n_samples, n_samples_valid))
            with open(anno_processed_path, 'w') as ff:
                json.dump(self.annotations, ff)

        print('Data split {}, with {} samples'.format(
            self.split, len(self.annotations)))
    ...
    def _get_query_path(self, sample):
        clip_name = sample['clip_uid']
        # "{}/frame_{:07d}.jpg"
        image_name = int(sample["visual_crop"]["frame_number"])
        image_path = os.path.join(
            self.data_dir, 'images_hw', "{}/frame_{:07d}.jpg".format(clip_name, image_name + 1))
        return image_path
    ...
    def _get_clip_bbox(self, sample, clip_idxs):
        ...
        for it in response_track:
            clip_bbox_all[int(it['frame_number'])] = [
                it['y'], it['x'], it['y'] + it['height'], it['x'] + it['width']]  # in torch
        ...
        for id in clip_idxs:
            if int(id) in clip_bbox_all.keys():
                ...
                cur_bbox_normalize = dataset_utils.normalize_bbox(
                    cur_bbox, origin_hw[0], origin_hw[1])
    ...
    def _get_query(self, sample, query_path):
        ...
        # BoxMode.XYXY_ABS, for crop only
        bbox = get_bbox_from_data(sample["visual_crop"])
        ...
        if self.query_params['query_padding']:
            ...
            query = F.interpolate(query.unsqueeze(0), size=(
                query_size, query_size), mode='bilinear').squeeze(0)
        else:
            query_size = self.query_params['query_size']
            query = query.resize((query_size, query_size))
            query = torch.from_numpy(np.asarray(
                query) / 255.0).permute(2, 0, 1)  # RGB, [C,H,W]
        return query
    ...
    def _process_clip(self, clip, clip_bbox, clip_with_bbox):
        ...
        if pad_height:
            # for the left, top, right and bottom borders respectively
            pad_input = [0, pad_size] * 2
            clip_bbox[:, 0] += (max_size - min_size) / \
                2.0   # in padded image size
        ...
        clip = F.interpolate(clip, size=(
            target_size, target_size), mode='bilinear')  # .squeeze(0)
    ...
    def __getitem__(self, idx):
        ...
        clip, clip_bbox, clip_with_bbox, query, clip_h, clip_w = self._process_clip(
            clip, clip_bbox, clip_with_bbox)
        ...
        query_frame, query_frame_bbox = self._get_query_frame(
            sample, query_path)
        ...
        results = {
            ...
            # [T,4]
            'clip_bbox': clip_bbox.float().clamp(min=0.0, max=1.0),
...
def sample_frames_balance(num_frames, query_frame, frame_interval, sample, sampling='rand'):
    ...
    if anno_len <= required_len:
        ...
        if anno_valid_idx_range[1] < required_len:
            idx_start = random.choice(
                range(anno_valid_idx_range[0])) if anno_valid_idx_range[0] > 0 else 0
            idx_end = idx_start + required_len
        else:
            num_prior = random.choice(
                range(num_invalid)) if num_invalid != 0 else 0
        ...
        intervals = np.linspace(
            start=idx_start, stop=idx_end, num=num_frames+1).astype(int)
        ranges = []
        ...
        frame_idxs_pos = [anno_valid_idx_range[0] +
                          start + it for it in range(num_frames)]
...
def read_frames_decord_balance(video_path, num_frames, frame_interval, sample, sampling='rand'):
    ...
    # downsampled fps idxs, used to get bbox annotation
    frame_idxs = sample_frames_balance(
        num_frames, query_frame, frame_interval, sample, sampling)
    before_query = torch.tensor(frame_idxs) < query_frame
    frame_idxs_origin = [min(it * down_rate, vlen - 1)
                         for it in frame_idxs]        # origin clip fps frame idxs
...
video_reader_dict = {
    ...
    'decord_random': read_frames_decord_balance,
}
