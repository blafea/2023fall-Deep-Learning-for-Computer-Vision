...
import itertools
import argparse
from base_dataset import QueryVideoDataset
import kornia
...
def process_data(config, sample, iter=0, split='train', device='cuda'):
    ...
    normalization = kornia.enhance.Normalize(
    ...
    transform_clip = K.AugmentationSequential(
        K.ColorJitter(brightness=0.4, contrast=0.4,
                      saturation=0.3, hue=0, p=1.0),
        K.RandomHorizontalFlip(p=0.5),
        K.RandomResizedCrop((H, W), scale=(0.66, 1.0), ratio=(
            crop_ratio_min, crop_ratio_max), p=1.0),
        ...
        # Just to define the future input here.
    ...
    transform_query = K.AugmentationSequential(
        ...
        K.RandomResizedCrop((query_size, query_size), scale=(
            crop_sacle, 1.0), ratio=(crop_ratio_min, crop_ratio_max), p=prob_crop),
        # K.RandomAffine(affine_degree, [affine_translate, affine_translate], [affine_scale_min, affine_scale_max],
        #                 [affine_shear_min, affine_shear_max], p=prob_affine
    ...
    # [B,T,4], with value range [0,1], torch axis
    clip_bbox = sample['clip_bbox']
    # [B,T,4], with range in image pixels, torch axis
    clip_bbox = recover_bbox(clip_bbox, H, W)
    # [B,T,4], with range in image pixels, cv2 axis
    clip_bbox = bbox_torchTocv2(clip_bbox)
    ...
    if split == 'train' and config.train.aug_clip and (iter > config.train.aug_clip_iter):
        ...
        # [T,C,H,W], [T,4,2]
        for clip_cur, clip_bbox_cur in zip(clip, clip_bbox):
            clip_cur_aug, clip_bbox_cur_aug = transform_clip(
                clip_cur.to(device), clip_bbox_cur.to(device).unsqueeze(1))
        ...
        # back in range [0,1]
        clip_bbox_aug = normalize_bbox(clip_bbox_aug, H, W)
        clip_with_bbox_aug = torch.logical_and(
            with_bbox_update.to(clip_with_bbox.device), clip_with_bbox)
    ...
    if split == 'train' and config.train.aug_query and 'query_frame' in sample.keys():
        ...
        query_frame_bbox = normalize_bbox(
            query_frame_bbox, H, W).clamp(min=0.0, max=1.0)
    ...
    for i in range(b):
        for j in range(b):
            ...
            if i == j:
                ...
                new_clip_bbox.append(torch.tensor(
                    [[0.0, 0.0, 0.0001, 0.0001]]).repeat(t, 1).float().to(device))
...
def bbox_xyhwToxyxy(bbox_xyhw):
    ...
    bbox_xyxy = torch.cat(
        [bbox_center - bbox_hw_half, bbox_center + bbox_hw_half], dim=-1)
    return bbox_xyxy
