...
def train_epoch(config, loader, model, optimizer, schedular, scaler, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    for batch_idx, sample in enumerate(tqdm(loader)):
        ...
        # normalize and data augmentations on GPU
        ...
        if config.train.use_query_roi and 'query_frame' in sample.keys():
            ...
            preds = model(clips, queries, training=True,
                          fix_backbone=config.model.fix_backbone)
        time_meters.add_loss_value('Prediction time', time.time() - end)
        end = time.time()

        losses, preds_top, sample = loss_utils.get_losses_with_anchor(
            config, preds, sample)
        total_loss = 0.0
...
def validate(config, loader, model, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(tqdm(loader)):
            ...
            if config.train.use_query_roi and 'query_frame' in sample.keys():
                ...
                preds = model(clips, queries, training=False,
                              fix_backbone=config.model.fix_backbone)
    ...
    if rank == 0:
        ...
        for k in metrics.keys():
            wandb_log['Valid/{}'.format(k)
                      ] = torch.tensor(metrics[k]).mean().item()
        # wandb_run.log(wandb_log)

    return torch.tensor(metrics['iou_25']).mean().item(), torch.tensor(metrics['prob_accuracy']).mean().item()
...
def val_performance(config, preds, gts, prob_theta=0.5):
    ...
    pred_bbox = torch.gather(pred_bbox, dim=1, index=top_idx.unsqueeze(
        -1).unsqueeze(-1).repeat(1, 1, 4)).squeeze()       # [b*t,4]
    pred_hw = torch.gather(pred_hw, dim=1, index=top_idx.unsqueeze(
        -1).unsqueeze(-1).repeat(1, 1, 2)).squeeze()           # [b*t,2]
    pred_center = torch.gather(pred_center, dim=1, index=top_idx.unsqueeze(
        -1).unsqueeze(-1).repeat(1, 1, 2)).squeeze()   # [b*t,2]
    ...
    iou_all, giou, loss_giou = GiouLoss(
    ...
    prob_accuracy = ((torch.sigmoid(pred_prob) > prob_theta)
                     == gt_prob.bool()).float().mean()
    prob_accuracy_2 = ((torch.sigmoid(pred_prob) > 0.6)
                       == gt_prob.bool()).float().mean()
    prob_accuracy_3 = ((torch.sigmoid(pred_prob) > 0.7)
                       == gt_prob.bool()).float().mean()
    prob_accuracy_4 = ((torch.sigmoid(pred_prob) > 0.65)
                       == gt_prob.bool()).float().mean()
