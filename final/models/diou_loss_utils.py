...
def get_losses_with_anchor(config, preds, gts):
    ...
    if 'center' not in gts.keys():
        gts['center'] = (gts['clip_bbox'][..., :2] +
                         gts['clip_bbox'][..., 2:]) / 2.0
    if 'hw' not in gts.keys():
        gts['hw'] = gts['center'] - \
            gts['clip_bbox'][..., :2]   # actually half of hw
    ...
    if torch.sum(positive.float()).item() == 0:
        positive[:1] = True
    loss_mask = positive.float().unsqueeze(
        1)                                    # [b*t*N,1]
    ...
    if torch.sum(positive.float()).item() > 0:
        ...
        gt_center_positive = rearrange(gt_center.unsqueeze(2).repeat(
            1, 1, N, 1), 'b t N c -> (b t N) c')[positive.bool()]
        ...
        gt_hw_positive = rearrange(gt_hw.unsqueeze(2).repeat(
            1, 1, N, 1), 'b t N c -> (b t N) c')[positive.bool()]
        ...
        gt_bbox_replicate = rearrange(gt_bbox.unsqueeze(
            2).repeat(1, 1, N, 1), 'b t N c -> (b t N) c')
        #iou, giou, loss_giou = GiouLoss(
        #    pred_bbox, gt_bbox_replicate, mask=loss_mask.bool().squeeze())
        iou, diou, loss_diou = DiouLoss(pred_bbox, gt_bbox_replicate, mask=loss_mask.bool().squeeze())
    else:
        pred_bbox = rearrange(pred_bbox, 'b t N c -> (b t N) c')
        loss_center = torch.tensor(0.).cuda()
        loss_hw = torch.tensor(0.).cuda()
        loss_diou = torch.tensor(0.).cuda()
        iou = torch.tensor(0.).cuda()
        diou = torch.tensor(0.).cuda()
    ...
    # else:
    pred_prob = rearrange(pred_prob, 'b t N -> (b t N)')
    gt_before_query_replicate = rearrange(
        gt_before_query.unsqueeze(2).repeat(1, 1, N), 'b t N -> (b t N)')
    ...
    # get top prediction
    # [b*t,N]
    pred_prob = rearrange(pred_prob, '(B N) -> B N', N=N)
    pred_bbox = rearrange(pred_bbox, '(B N) c -> B N c',
                          N=N)                                   # [b*t,N,4]
    # [b*t], [b*t]
    pred_prob_top, top_idx = torch.max(pred_prob, dim=-1)
    pred_bbox_top = torch.gather(pred_bbox, dim=1, index=top_idx.unsqueeze(
        -1).unsqueeze(-1).repeat(1, 1, 4)).squeeze()   # [b*t,4]
    pred_top = {
...
def get_losses_head(config, refine_prob, gts, preds_top):
    ...
    iou, diou, loss_diou = DiouLoss(pred_bbox, gt_bbox)
...
def get_losses(config, preds, gts):
    ...
    if 'hw' not in gts.keys():
        gts['hw'] = gts['center'] - \
            gts['clip_bbox'][..., :2]   # actually half hw
    ...
    loss_center = F.l1_loss(
        pred_center[gt_prob.bool()], gt_center[gt_prob.bool()])
    ...
    iou, diou, loss_diou = DiouLoss(pred_bbox, gt_bbox, mask=gt_prob.bool())
    if 'bbox_ratio' in preds.keys():
        pred_ratio = preds['bbox_ratio'].reshape(-1)
        loss_ratio = F.l1_loss(
            pred_ratio[gt_prob.bool()], gt_ratio[gt_prob.bool()])

    # occurance loss
    weight = torch.tensor(config.loss.prob_bce_weight).to(gt_prob.device)
    weight_ = weight[gt_prob[gt_before_query.bool()].long()].reshape(-1)
    criterion = nn.BCEWithLogitsLoss(reduce=False)
    loss_prob = (criterion(pred_prob[gt_before_query.bool(
    )], gt_prob[gt_before_query.bool()]) * weight_).mean()
...
def GiouLoss(bbox_p, bbox_g, mask=None):
    ...
    I = torch.maximum((y2I - y1I), torch.tensor([0.0]).to(device)) * torch.maximum(
        (x2I - x1I), torch.tensor([0.0]).to(device))
...
def DiouLoss(bbox_p, bbox_g, mask=None):
    """
    :param bbox_p: predict of bbox(N,4)(x1,y1,x2,y2)
    :param bbox_g: groundtruth of bbox(N,4)(x1,y1,x2,y2)
    :param mask: ground truth of valid instance, in shape [B]
    ...
    center_x_p, center_y_p = calculate_bbox_center(bbox_p)  # torch.Size([N])
    center_x_g, center_y_g = calculate_bbox_center(bbox_g)  # torch.Size([N])
    ...
    # find enclosing box
    x1C = torch.minimum(bbox_p[:, 0], bbox_g[:, 0])   # torch.Size([N])
    y1C = torch.minimum(bbox_p[:, 1], bbox_g[:, 1])   # torch.Size([N])
    x2C = torch.maximum(bbox_p[:, 2], bbox_g[:, 2])   # torch.Size([N])
    y2C = torch.maximum(bbox_p[:, 3], bbox_g[:, 3])   # torch.Size([N])
    ...
    # 最小閉包區域的對角線距離
    diagonal_distance = (x2C - x1C)**2 + (y2C - y1C)**2   # torch.Size([N]) == c^2

    # 預測框的中心和ground truth框的中心之間的歐式距離
    center_distance = (center_x_p - center_x_g)**2 + (center_y_p - center_y_g)**2 # torch.Size([N]) == p^2
    penalty = center_distance / (diagonal_distance + 1e-6)

    # Diou
    diou  = iou - penalty

    if torch.is_tensor(mask):
        loss_diou = torch.mean(1.0 - diou[mask])
    else:
        loss_diou = torch.mean(1.0 - diou)

    return iou, diou, loss_diou
...
#     ratio = h / w
#     distance = torch.abs(ratio.repeat(
#         1, default_ratios.shape[0]) - default_ratios.unsqueeze(0))     # [b,n]
#     idx = torch.argmax(distance, dim=-1)    # [b]
#     ratio_quant = torch.tensor([default_ratios[it.long()]
#                                for it in idx]).to(device)
#     return ratio_quant
...
def BCELogitsLoss_with_HNM(pred_prob, gt_prob, positive, gt_before_query, weight):
    ...
    BCE_loss = F.binary_cross_entropy_with_logits(
        pred_prob, gt_prob, reduction='none')  # [b*t*N]
    ...
    loss = HardNegMining(pred_prob, gt_prob, positive,
                         BCE_loss, gt_before_query, weight)
    return loss.mean()
...
def HardNegMining(pred_prob, gt_prob, positive, BCE_loss, gt_before_query, weight, ratio_neg_pos=3., ratio_hard=0.05):
    ...
    '''
    b, t, N = pred_prob.shape
    # for example, real batch size B will lead to B^2 training batch leveraging cross-video data
    b_real = int(b ** 0.5)
    w_pos, w_neg = weight
    ...
    for i in range(b_real):
        ...
        # corresponds to how we generate cross-video data
        query_idx = [(i + j * b_real) for j in range(b_real)]

        # [b_real, t]
        cur_gt_before_query = gt_before_query[query_idx].bool()

        # [M], for all valid anchor box of the query (reject unreliable ones after query time)
        cur_positive = positive[query_idx][cur_gt_before_query]
        cur_loss = BCE_loss[query_idx][cur_gt_before_query]         # [M]
        M = cur_loss.shape[0]

        num_pos = int(torch.sum(cur_positive).item())
        num_neg = int(ratio_neg_pos *
                      num_pos) if num_pos > 0 else int(ratio_hard * M)
