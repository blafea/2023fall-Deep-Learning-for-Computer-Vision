...
class ClipMatcher(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.pe_3d_2 = positionalencoding3d(d_model=256,
                height=16,
                width=16,
                depth=30,
                type='zero').unsqueeze(0)
        self.pe_3d_2 = nn.parameter.Parameter(self.pe_3d_2)
    ...
    def forward(self, clip, query, query_frame_bbox=None, training=False, fix_backbone=True):
        ...
        for head in self.down_heads:
            ...
            # 16*2
            if list(clip_feat.shape[-2:]) == [self.resolution_transformer*2]*2:
               clip_feature = copy.copy(clip_feat)
               clip_feature = rearrange(clip_feature, '(b t) c h w -> b (t h w) c', b=b) + self.pe_3d_2

               mask_feature = self.get_mask(clip_feature, t)
               for layer in self.feat_corr_transformer:
                   clip_feature = layer(clip_feature, src_mask=mask_feature)
               clip_feature = rearrange(clip_feature, 'b (t h w) c -> (b t) c h w', b=b,t=t, h=self.resolution_transformer*2, w=self.resolution_transformer*2)
        ...
        # [b*t,N=h*w*n*m,c]
        bbox_refine, prob = self.head(clip_feat,clip_feature)
        ...
        hw = 0.5 * hw
        bbox = torch.cat([center - hw, center + hw], dim=-
                         1)                    # [b,t,N,4]
...
class Head(nn.Module):
    def __init__(self, in_dim=256, in_res=8, out_res=16, n=n_base_sizes, m=n_aspect_ratios):
        ...
        self.num_layers = 3
        self.latlayer1 = CBAM(channel = in_dim)
        self.latlayer2 = CBAM(channel = in_dim)
    ...
    def forward(self, x, feature):
        ...
        '''
        if self.num_up_layers > 0:
            x = self.up_convs(x) + self.latlayer1(feature)    # [B,c,h=16,w=16]
        ...
        for module in self.regression_conv:
            feat_reg = module(feat_reg)
            feat_reg = self.latlayer2(feat_reg)
