...
def get_losses_with_anchor(config, preds, gts):
    ...
    if gt_prob.bool().any():
        assign_label = assign_labels(anchor.repeat(b, t, 1, 1), gt_bbox,   # anchor.repeat(b,t,1,1) / pred_bbox
                                     ...
                                     nwd_threshold=thetas, # Add this line for nwd threshold
    ...
    if torch.sum(positive.float()).item() > 0:
        ...
        # loss_center_reg = F.l1_loss(pred_reg[:,:,:2], gt_center)
        # loss_hw_reg = F.l1_loss(pred_reg[:,:,2:], gt_hw)
        # loss_reg = loss_center_reg + loss_hw_reg
        
        # bbox NWD loss
        pred_cchw_postive = torch.cat([pred_center_positive, pred_hw_positive], dim=-1)
        gt_cchw_postive = torch.cat([gt_center_positive, gt_hw_positive], dim=-1)
        loss_nwd = torch.mean(1 - wasserstein_loss(pred_cchw_postive, gt_cchw_postive))
    ...
    loss = {
        ...
        # 'weight_reg': config.loss.weight_reg,
        'weight_nwd': config.loss.weight_nwd,
        # information
        'iou': iou.detach(),
        'giou': giou.detach()
    }
