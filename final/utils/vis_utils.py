...
def vis_pred_clip(sample, pred, iter_num, output_dir, subfolder='train'):
    ...
    for i in range(B):
        frames = []
        # [T,3,H,W], [3,H2,W2]
        cur_clip, cur_query = clip[i], query[i]
        cur_bbox, cur_bbox_pred = bbox[i], bbox_pred[i].clamp(
            min=0.0, max=1.0)     # [T,4]
        # [T]
        cur_prob, cur_prob_pred = prob[i], prob_pred[i]

        cur_query = cur_query.clamp(min=0.0, max=1.0).permute(
            1, 2, 0).numpy()        # [H2,W2,3]
        for j in range(T):
            ...
            fig.suptitle('Prob: gt {:.3f}, pred {:.3f}'.format(
                cur_prob[j].item(), torch.sigmoid(cur_prob_pred[j]).item()), fontsize=20)
            ax[0].imshow(img)
            ax[1].imshow(cur_query)
            if cur_prob[j].item() > 0.5:
                draw_bbox_gt = dataset_utils.recover_bbox(
                    cur_bbox[j], H, W)  # [4]
                rect = patches.Rectangle((draw_bbox_gt[1], draw_bbox_gt[0]),
                                         draw_bbox_gt[3] -
                                         draw_bbox_gt[1], draw_bbox_gt[2] -
                                         draw_bbox_gt[0],
                                         linewidth=1, edgecolor='r', facecolor='none')
                ax[0].add_patch(rect)
            if cur_prob[j].item() > 0.5:
                draw_bbox_pred = dataset_utils.recover_bbox(
                    cur_bbox_pred[j], H, W)  # [4]
                rect = patches.Rectangle((draw_bbox_pred[1], draw_bbox_pred[0]),
                                         draw_bbox_pred[3] -
                                         draw_bbox_pred[1], draw_bbox_pred[2] -
                                         draw_bbox_pred[0],
            ...
            plt.close()
            frames.append(cv2.imread(os.path.join(
                output_dir, 'tmp.png'))[..., ::-1])
...
def vis_pred_scores(sample, pred, iter_num, output_dir, subfolder='train'):
    ...
    for i in range(B):
        cur_prob, cur_prob_pred = prob[i].numpy(), torch.sigmoid(
            prob_pred[i]).numpy()     # [T]
        x = np.arange(T)
...
def vis_pred_clip_inference(clips, queries, pred, save_path, iter_num):
    ...
    for i in range(T):
        ...
        if cur_prob.item() > 0.5:
            # dataset_utils.recover_bbox(cur_bbox, H, W)  # [4]
            draw_bbox_pred = cur_bbox
            rect = patches.Rectangle((draw_bbox_pred[0], draw_bbox_pred[1]),
                                     draw_bbox_pred[2] -
                                     draw_bbox_pred[0], draw_bbox_pred[3] -
                                     draw_bbox_pred[1],
                                     linewidth=1, edgecolor='b', facecolor='none')
            ax[0].add_patch(rect)
