...
from dataset_utils import bbox_xyhwToxyxy
...
base_sizes = torch.tensor([[16, 16], [32, 32], [64, 64], [
                          128, 128]], dtype=torch.float32)    # 4 types of size
# 3 types of aspect ratio
...
def build_backbone(config):
    ...
    if name == 'dino':
        ...
        backbone = torch.hub.load(
            'facebookresearch/dino:main', 'dino_{}'.format(type))
        down_rate = int(type.replace('vitb', '').replace('vits', ''))
        backbone_dim = 768
        if type == 'vitb16' and config.model.bakcbone_use_mae_weight:
            mae_weight = torch.load(
                '/vision/hwjiang/episodic-memory/VQ2D/checkpoint/mae_pretrain_vit_base.pth')['model']
            backbone.load_state_dict(mae_weight)
    elif name == 'dinov2':
        assert type in ['vits14', 'vitb14', 'vitl14', 'vitg14', 'vits14_reg', 'vitb14_reg', 'vitl14_reg', 'vitg14_reg']
        backbone = torch.hub.load(
            'facebookresearch/dinov2', 'dinov2_{}'.format(type))
        down_rate = 14
        if type == 'vitb14' or type == 'vitb14_reg':
            backbone_dim = 768
        elif type == 'vits14' or type == 'vits14_reg':
            backbone_dim = 384
        elif type == 'vitl14' or type == 'vitl14_reg':
            backbone_dim = 1024
        elif type == 'vitg14' or type == 'vitg14_reg':
            backbone_dim = 1536
    elif name == 'mae':
        backbone = vit_base_patch16()
        cpt = torch.load(
            '/vision/hwjiang/download/model_weight/mae_pretrain_vit_base.pth')['model']
...
class ClipMatcher(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.backbone, self.down_rate, self.backbone_dim = build_backbone(
            config)
        ...
        # [R^2*N*M,4], value range [0,1], represented by [c_x,c_y,h,w] in torch axis
        self.anchors_xyhw = self.anchors_xyhw / self.clip_size_coarse
        ...
        for _ in range(int(math.log2(self.query_feat_size))):
            self.query_down_heads.append(
                nn.Sequential(
                    nn.Conv2d(self.backbone_dim, self.backbone_dim,
        ...
        self.num_head_layers, self.down_heads = int(
            math.log2(self.clip_feat_size_coarse)), []
        ...
        # output head
        self.head = Head(in_dim=256, in_res=self.resolution_transformer,
                         out_res=self.resolution_anchor_feat)

        # Post clip-query correspondence attention (baseliner)
        # =================================================================
        # self.post_CQ_corr_transformer = []
        # self.post_Query_proj = nn.Linear(1024, 64)
        # for _ in range(1):
        #     self.post_CQ_corr_transformer.append(
        #         torch.nn.TransformerDecoderLayer(
        #             d_model=256,
        #             nhead=4,
        #             dim_feedforward=1024,
        ...
        #         )
        #     )
        # self.post_CQ_corr_transformer = nn.ModuleList(
        #     self.post_CQ_corr_transformer)
        # ================================================================


        # Post decoder layers (baseliner)
        # =================================================================
        # self.post_decoder_layer = []
        # self.post_head = nn.Sequential(
        #     BasicBlock_Conv2D(256, 128),
        #     nn.MaxPool2d(2),
        #     BasicBlock_Conv2D(128, 64),
        #     nn.MaxPool2d(2),
        # )
        ...
        # )
        # for _ in range(self.num_transformer):
        #     self.post_decoder_layer.append(
        #         torch.nn.TransformerDecoderLayer(
        #             d_model=256,
        #             nhead=4,
        #             dim_feedforward=1024,
        ...
        #         )
        #     )
        # self.post_decoder_layer = nn.ModuleList(
        #     self.post_decoder_layer)
        # ================================================================
    ...
    def extract_feature(self, x, return_h_w=False):
        if self.backbone_name == 'dino':
            ...
            h, w = int(h_origin / self.backbone.patch_embed.patch_size), int(
                w_origin / self.backbone.patch_embed.patch_size)
            dim = out.shape[-1]
            ...
            b, _, h_origin, w_origin = x.shape
            out = self.backbone.get_intermediate_layers(x, n=1)[0]
            h, w = int(h_origin / self.backbone.patch_embed.patch_size[0]), int(
                w_origin / self.backbone.patch_embed.patch_size[1])
            dim = out.shape[-1]
            out = out.reshape(b, h, w, dim).permute(0, 3, 1, 2)
            if return_h_w:
                return out, h, w
            ...
            h, w = int(h_origin / self.backbone.patch_embed.patch_size[0]), int(
                w_origin / self.backbone.patch_embed.patch_size[1])
            dim = out.shape[-1]
            out = out[:, 1:].reshape(b, h, w, dim).permute(
                0, 3, 1, 2)  # [b,c,h,w]
    ...
    def forward(self, clip, query, query_frame_bbox=None, training=False, fix_backbone=True):
        ...
        if torch.is_tensor(query_frame_bbox) and self.config.train.use_query_roi:
            idx_tensor = torch.arange(
                b, device=clip.device).float().view(-1, 1)
            query_frame_bbox = dataset_utils.recover_bbox(
                query_frame_bbox, h, w)
            roi_bbox = torch.cat([idx_tensor, query_frame_bbox], dim=1)
            query_feat = torchvision.ops.roi_align(
                query_feat, roi_bbox, (h, w))

        # reduce channel size,  c ->  c/3
        ...
        if self.config.train.use_hnm and training:
            clip_feat, query_feat = self.replicate_for_hnm(
                query_feat, clip_feat)   # b -> b^2
            b = b**2

        # find spatial correspondence between query-frame
        query_feat = rearrange(query_feat.unsqueeze(1).repeat(
            1, t, 1, 1, 1), 'b t c h w -> (b t) (h w) c')  # [b*t,n,c]
        # [b*t,n,c]
        clip_feat = rearrange(clip_feat, 'b c h w -> b (h w) c')
        ...
        # clip_feat_post1 = clip_feat.clone().requires_grad_()
        ...
        for head in self.down_heads:
            ...
            if list(clip_feat.shape[-2:]) == [self.resolution_transformer]*2:
                clip_feat = rearrange(
                    clip_feat, '(b t) c h w -> b (t h w) c', b=b) + self.pe_3d
                mask = self.get_mask(clip_feat, t)
                for layer in self.feat_corr_transformer:
                    clip_feat = layer(clip_feat, src_mask=mask)
                clip_feat = rearrange(clip_feat, 'b (t h w) c -> (b t) c h w', b=b,
                                      t=t, h=self.resolution_transformer, w=self.resolution_transformer)
                break

        # Post clip-query correspondence attention (baseliner)
        # =================================================================
        # query_feat = rearrange(query_feat, 'b c s -> b s c')
        # query_feat = self.post_Query_proj(query_feat)
        # query_feat = rearrange(query_feat, 'b s c -> b c s')
        # clip_feat = rearrange(clip_feat, 'b c h w -> b (h w) c')
        # for layer in self.post_CQ_corr_transformer:
        #     # [b*t,n,c]
        #     # clip_feat = layer(clip_feat, query_feat) # clip as q, query as k,v (X)
        #     clip_feat = layer(query_feat, clip_feat) # query as q, clip as k,v
        # # [b*t,c,h,w]
        # clip_feat = rearrange(clip_feat, 'b (h w) c -> b c h w',
        #                       h=self.resolution_transformer, w=self.resolution_transformer)
        # =================================================================


        # Post decoder layers (baseliner)
        # Idea: use cross attention between pre_clip and post_clip, directly predict xxyyc
        # =================================================================
        # clip_feat_post2 = clip_feat.clone().requires_grad_()
        # clip_feat_post1 = rearrange(clip_feat_post1, 'b c h w -> b (h w) c')
        # clip_feat_post2 = rearrange(clip_feat_post2, 'b c h w -> b (h w) c')
        # for layer in self.post_decoder_layer:
        #     # [b*t,n,c]
        #     clip_feat_post2 = layer(clip_feat_post1, clip_feat_post2)
        # # [b*t,c,h,w]
        # clip_feat_post2 = rearrange(clip_feat_post2, 'b (h w) c -> b c h w', h=h, w=w)
        # clip_feat_post2 = self.post_head(clip_feat_post2)
        # clip_feat_post2 = rearrange(clip_feat_post2, 'b h w c -> b (h w c)')
        # reg = self.post_ffn(clip_feat_post2) 
        # reg = rearrange(reg, '(b t) c -> b t c', b=b, c=4) # [b,t,4]
        # =================================================================

        # refine anchors
        anchors_xyhw = self.anchors_xyhw.to(
            clip_feat.device)                   # [N,4]
        anchors_xyxy = self.anchors_xyxy.to(
            clip_feat.device)                   # [N,4]
        anchors_xyhw = anchors_xyhw.reshape(
            1, 1, -1, 4)                           # [1,1,N,4]
        anchors_xyxy = anchors_xyxy.reshape(
            1, 1, -1, 4)                           # [1,1,N,4]

        # [b*t,N=h*w*n*m,c]
        bbox_refine, prob = self.head(clip_feat)
        # [b,t,N,4], in xyhw frormulation
        bbox_refine = rearrange(bbox_refine, '(b t) N c -> b t N c', b=b, t=t)
        prob = rearrange(prob, '(b t) N c -> b t N c', b=b,
                         t=t)                # [b,t,N,1]
        # [b,t,N,4]
        bbox_refine += anchors_xyhw

        # represented by [c_x, c_y, h, w]
        center, hw = bbox_refine.split([2, 2], dim=-1)
        # anchor's hw is defined as real hw
        hw = 0.5 * hw
...
class Head(nn.Module):
    def __init__(self, in_dim=256, in_res=8, out_res=16, n=n_base_sizes, m=n_aspect_ratios):
        ...
        if self.num_up_layers > 0:
            ...
            for _ in range(self.num_up_layers):
                self.up_convs.append(torch.nn.ConvTranspose2d(
                    in_dim, in_dim, kernel_size=4, stride=2, padding=1))
        ...
        self.regression_head = nn.Conv2d(
            in_dim, n * m * 4, kernel_size=3, padding=1)
        self.classification_head = nn.Conv2d(
            in_dim, n * m * 1, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        feat_reg, feat_cls = self.in_conv(x).split(
            [c, c], dim=1)   # both [B,c,h,w]
        ...
        out_reg = rearrange(
            out_reg, 'B (n m c) h w -> B (h w n m) c', h=h, w=w, n=self.n, m=self.m, c=4)
        out_cls = rearrange(
            out_cls, 'B (n m c) h w -> B (h w n m) c', h=h, w=w, n=self.n, m=self.m, c=1)
