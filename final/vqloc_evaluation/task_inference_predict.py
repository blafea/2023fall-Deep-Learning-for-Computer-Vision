...
class Task:
    def __init__(self, config, annots):
        ...
        # self.keys = [
        #     (annot["metadata"]["annotation_uid"],
        #      annot["metadata"]["query_set"])
        #     for annot in self.annots
        # ]
    ...
    def run(self, model, config, device):
        ...
        for annot in self.annots:
            ...
            ret_bboxes, ret_scores = inference_video(config,
                                                     ...
                                                     visual_crop,
                                                     os.path.join(
                                                         self.config.inference_cache_path, annot_key),
                                                     device)
...
def inference_video(config, model, clip_path, query_frame, visual_crop, save_path, device):
    ...
    for i in range(inference_time):
        ...
        # get current clips
        clips_origin, clips = load_clip(
            config, clip_reader, frame_idx, clip_path)    # [N,3,H,W]
        clips_origin = clips_origin[:num_frames]
        clips = rearrange(clips, '(b t) c h w -> b t c h w',
                          b=batch_size_inference, t=clip_num_frames)
        ...
        try:
            ...
            print(clips.shape, idx_start, idx_end, batch_size_inference,
                  inference_num_frames, query_frame, inference_time)
        clips = clips.to(device)
        queries = queries.to(device)

        # inference
        with torch.no_grad():
            preds = model(clips, queries,
...
def get_top_predictions(config, preds, num_frames, oshape):
    ...
    pred_bbox_processed = process_bbox_prediction(
        pred_bbox, owidth, oheight, resize_res)
...
def process_prob(top_idx, top_prob, preds):
    ...
    top_bbox = torch.gather(pred_bbox, dim=1, index=top_idx.unsqueeze(
        -1).unsqueeze(-1).repeat(1, 1, 4)).squeeze()  # [b*t,4]
