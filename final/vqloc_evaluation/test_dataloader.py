...
import numpy as np
import kornia
from dataset_utils import NORMALIZE_MEAN, NORMALIZE_STD
from einops import rearrange
import matplotlib.pyplot as plt

from base_dataset import get_bbox_from_data
...
def load_query(config, clip_reader, visual_crop, clip_path):
    ...
    if config.dataset.query_padding:
        ...
        # print(query)
        plt.imsave("aaa.jpg", (query*255).permute(1,
...
def load_clip(config, clip_reader, frame_idx, clip_path):
    ...
    clips = F.interpolate(clips, size=(
        target_size, target_size), mode='bilinear')
