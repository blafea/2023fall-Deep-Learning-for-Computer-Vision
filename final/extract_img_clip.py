...
def visualize_query_set(video_reader, qset, save_height=360):
    ...
    for rf_fno, rf_box in zip(rt_fnos, rt_boxes):
        ...
        rf_im = draw_box_on_image(
            rf_frame, rf_box, color=(0, 255, 0), thickness=2)
...
if __name__ == "__main__":
    ...
    for cuid, c in annotations.items():
        # cuid = c["clip_uid"]
        os.makedirs(f"{args.vis_save_root}/{cuid}", exist_ok=True)
        for a_idx, a in enumerate(c["annotations"]):
            for qset_id, qset in a["query_sets"].items():
                ...
                csp = f"{args.vis_save_root}/{cuid}/frame_{vc_fno+1:07d}.jpg"
