...
from vqloc_evaluation import eval_utils
from vqloc_evaluation.task_inference_predict import Task
...
class WorkerWithDevice(mp.Process):
    ...
    def work(self, task_queue, results_queue):
        ...
        print('Model with {} parameters'.format(sum(p.numel()
...
if __name__ == '__main__':
    ...
    config.inference_cache_path = os.path.join(
        output_dir, f'inference_cache_{mode}')
    ...
    annotation_path = os.path.join(
        'student_data', 'vq_{}.json'.format(mode))
    with open(annotation_path) as fp:
        annotations = json.load(fp)
    clipwise_annotations_list = eval_utils.convert_annotations_to_clipwise_list(
        annotations)
