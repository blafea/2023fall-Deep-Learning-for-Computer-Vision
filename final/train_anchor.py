...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Train hand reconstruction network')
    parser.add_argument(
...
def main():
    # Get args and config
    args = parse_args()
    logger, output_dir, tb_log_dir = exp_utils.create_logger(
        config, args.cfg, phase='train')
    ...
    #     # , name='smooth-puddle-94', resume=True)
    ...
    # load checkpoint and fix trained layers, remove this part if not needed
    model.load_state_dict(torch.load('cpt_best_prob.pth.tar')['state_dict'], strict=False)
    # for name, param in model.named_parameters():
    #     if 'post_' not in name:
    #         param.requires_grad = False
    train_param = sum(p.numel() for p in model.parameters())//1e6
    total_param = sum(p.numel() for p in model.parameters() if p.requires_grad)//1e6
    print('=====================================')
    print('Total parameters: {}M'.format(train_param))
    print('Trainable parameters: {}M'.format(total_param))
    print('Train Ratio: {:.2f}%'.format(total_param/train_param*100))
    print('=====================================')
    ...
    for epoch in range(start_ep, end_ep):
        ...
        logger.info('Rank {}, best iou: {} (current {}), best probability accuracy: {} (current {})'.format(
            local_rank, best_iou, iou, best_prob, prob))
        # dist.barrier()
        torch.cuda.empty_cache()
